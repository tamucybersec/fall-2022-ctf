...
There was already a solve script in the initial commit but I made a simpler one, solve in `solve.py` and below.
``` py
import requests
...
The only suggestion I'd have is provide a charset to reduce the burden of brute force, but that may give away the challenge
